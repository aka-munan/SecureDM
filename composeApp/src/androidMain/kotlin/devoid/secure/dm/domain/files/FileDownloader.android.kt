...
actual object AttachmentDownloader {
    actual fun downloadFile(attachment: MessageAttachment, overrideDuplicate: Boolean) = flow<DownloadStatus>{
        val httpClient = HttpClient()
        try {
            val path = File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS),"SecureDm/"+attachment.type.name)
